...
function createAction() {
	createDialog = $.dialog({
		animationSpeed: 300,
		title: '新增角色',
		content: 'url:${basePath}/upms/role/create',
...
function updateAction() {
	...
	if (rows.length != 1) {
		...
		updateDialog = $.dialog({
			animationSpeed: 300,
			title: '编辑角色',
			content: 'url:${basePath}/upms/role/update/' + rows[0].roleId,
			onContentReady: function () {
				initMaterialInput();
			}
		});
			...
			buttons: {
				confirm: {
					...
					action: function () {
						...
						$.ajax({
							type: 'get',
							url: '${basePath}/upms/role/delete/' + ids.join("-"),
...
function permissionAction() {
	...
	if (rows.length != 1) {
		...
		permissionDialog = $.dialog({
			animationSpeed: 300,
			title: '角色权限',
			content: 'url:${basePath}/upms/role/permission/' + roleId,
			onContentReady: function () {
				initMaterialInput();
				initTree();
			}
