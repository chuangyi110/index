...
$(function() {
	...
	$table.bootstrapTable({
		url: '${basePath}/upms/permission/list',
		height: getHeight(),
		striped: true,
		search: true,
		showRefresh: true,
		...
		columns: [
            ...
            {field: 'systemId',sortable: true,  title: '所属系统'},
			{field: 'pid', sortable: true, title: '所属上级'},
			{field: 'name',sortable: true,  title: '权限名称'},
			{field: 'type',sortable: true,  title: '类型', formatter: 'typeFormatter'},
			{field: 'permissionValue',sortable: true,  title: '权限值'},
			{field: 'uri', title: '路径'},
			{field: 'icon', title: '图标', align: 'center', formatter: 'iconFormatter'},
			{field: 'status',sortable: true,  title: '状态', sortable: true, align: 'center', formatter: 'statusFormatter'},
...
function createAction() {
	createDialog = $.dialog({
		animationSpeed: 300,
		title: '新增权限',
		content: 'url:${basePath}/upms/permission/create',
...
function updateAction() {
	...
	if (rows.length != 1) {
		...
		updateDialog = $.dialog({
			animationSpeed: 300,
			title: '编辑权限',
			content: 'url:${basePath}/upms/permission/update/' + rows[0].permissionId,
			onContentReady: function () {
				initMaterialInput();
				$('select').select2();
				initType();
			...
			buttons: {
				confirm: {
					...
					action: function () {
						...
						$.ajax({
							type: 'get',
							url: '${basePath}/upms/permission/delete/' + ids.join("-"),
