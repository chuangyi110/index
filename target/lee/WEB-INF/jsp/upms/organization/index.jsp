...
$(function() {
	...
	$table.bootstrapTable({
		url: '${basePath}/upms/organization/list',
		height: getHeight(),
		striped: true,
		search: true,
		showRefresh: true,
...
function createAction() {
	createDialog = $.dialog({
		animationSpeed: 300,
		title: '新增组织',
		content: 'url:${basePath}/upms/organization/create',
...
function updateAction() {
	...
	if (rows.length != 1) {
		...
		updateDialog = $.dialog({
			animationSpeed: 300,
			title: '编辑组织',
			content: 'url:${basePath}/upms/organization/update/' + rows[0].organizationId,
			onContentReady: function () {
				initMaterialInput();
			}
		});
			...
			buttons: {
				confirm: {
					...
					action: function () {
						...
						$.ajax({
							type: 'get',
							url: '${basePath}/upms/organization/delete/' + ids.join("-"),
