...
function createAction() {
	createDialog = $.dialog({
		animationSpeed: 300,
		title: '新增用户',
		content: 'url:${basePath}/upms/users/create',
...
function updateAction() {
	...
	if (rows.length != 1) {
		...
		updateDialog = $.dialog({
			animationSpeed: 300,
			title: '编辑用户',
			content: 'url:${basePath}/upms/users/update/' + rows[0].userId,
			onContentReady: function () {
				initMaterialInput();
                initUploader();
			}
			...
			buttons: {
				confirm: {
					...
					action: function () {
						...
						$.ajax({
							type: 'get',
							url: '${basePath}/upms/users/delete/' + ids.join("-"),
...
function organizationAction() {
	...
	if (rows.length != 1) {
		...
		organizationDialog = $.dialog({
			animationSpeed: 300,
			title: '用户组织',
			content: 'url:${basePath}/upms/users/organization/' + organizationUserId,
			onContentReady: function () {
				initMaterialInput();
				$('select').select2({
					placeholder: '请选择用户组织',
...
function roleAction() {
	...
	if (rows.length != 1) {
		...
		roleDialog = $.dialog({
			animationSpeed: 300,
			title: '用户角色',
			content: 'url:${basePath}/upms/users/role/' + roleUserId,
			onContentReady: function () {
				initMaterialInput();
				$('select').select2({
					placeholder: '请选择用户角色',
...
function permissionAction() {
	...
	if (rows.length != 1) {
		...
		permissionDialog = $.dialog({
			...
			title: '用户授权',
			columnClass: 'large',
			content: 'url:${basePath}/upms/users/permission/' + permissionUserId,
			onContentReady: function () {
				initMaterialInput();
				initTree();
			}
